...
class RepositoryManagerSpec extends ObjectBehavior
{
    ...
    {
        $this->addRepository(\stdClass::class, $repository);
    ...
    {
        $this->shouldThrow(RepositoryNotFoundException::class)->duringGetRepositoryForObject(new \stdClass());
    }

    function it_provides_the_repository_assigned_to_a_classname(Repository $repository)
    {
        $this->addRepository(\stdClass::class, $repository);
        $this->getRepositoryForClassName(\stdClass::class)->shouldReturn($repository);
    }

    function it_throws_an_exception_if_the_classname_has_no_repository_assigned()
    {
        $this->shouldThrow(RepositoryNotFoundException::class)->duringGetRepositoryForClassName(\stdClass::class);
    }
}
